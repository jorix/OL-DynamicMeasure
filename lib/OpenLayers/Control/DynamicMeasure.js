...
OpenLayers.Control.DynamicMeasure = OpenLayers.Class(OpenLayers.Control.Measure, {
    ...
    initialize: function(handler, options) {
        ...
        // * * styles for label layers.
        var createLayer = function(styleName, initialOptions) {
            ...
            if (!options.styleMap) {
                var style = optionsStyles[styleName];
            ...
            return new OpenLayers.Layer.Vector(
                                    this.CLASS_NAME + ' ' + styleName, options);
        };
        this.layerSegments = createLayer(
                                  "labelSegments", options.layerSegmentsOptions);
        this.layerLength = createLayer(
                                     "labelLength", options.layerLengthOptions);
        if (this.isArea) {
            this.layerArea = createLayer("labelArea", options.layerAreaOptions);
        }
        delete options.layerSegmentsOptions;
        delete options.layerLengthOptions;
        delete options.layerAreaOptions;
    ...
    activate: function() {
        ...
        if (response) {
            ...
            this.layerSegments && this.map.addLayers([this.layerSegments]);
            this.layerLength && this.map.addLayers([this.layerLength]);
            this.layerArea && this.map.addLayers([this.layerArea]);
            this.dynamicObj = {};
        }
        return response;
    },
    ...
    deactivate: function() {
        ...
        if (response) {
            var _removeLayer = function(layer) {
                ...
                layer.map && layer.map.removeLayer(layer);
            };
            _removeLayer(this.layerSegments);
            _removeLayer(this.layerLength);
            _removeLayer(this.layerArea);
        }
        return response;
    },
