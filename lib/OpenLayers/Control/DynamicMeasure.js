...
OpenLayers.Control.DynamicMeasure = OpenLayers.Class(
    ...
    callbackModify: function(point, feature, drawing) {
        ...
        if (this.layerLength) {
            this.showLabel(
                        this.layerLength, 1, 0, totalLength, positions.to(), 1);
        }
        if (this.isArea) {
            if (this.layerArea) {
                this.showLabel(this.layerArea, 1, 0,
                     this.getBestArea(feature.geometry), positions.center(), 1);
            }
    ...
    showLabelSegment: function(labelsNumber, fromIndex, points) {
        ...
        if (layerSegments) {
            created = this.showLabel(layerSegments, labelsNumber, fromIndex,
                           segmentLength, positions.middle(), this.maxSegments);
        }
        if (layerHeading && segmentLength[0] > 0) {
            ...
            created = created || this.showLabel(layerHeading,
                              ...
                              [bearing, '°'], positions.to(), this.maxHeadings);
        }
        return created;
    },
