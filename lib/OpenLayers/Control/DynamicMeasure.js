...
OpenLayers.Control.DynamicMeasure = OpenLayers.Class(
     ...
     * {Boolean} Activates the immediate measurement so that the
     *     "measurepartial" event is also fired once the measurement sketch is
    ...
    initialize: function(handler, options) {
        ...
        var handlerTuned = OpenLayers.Class(handler, {
            ...
            undo: function() {
                if (handler.prototype.undo.call(this)) {
                    this.callback('undo',
                                 [this.point.geometry, this.getSketch(), true]);
                }
            },
            redo: function() {
                if (handler.prototype.redo.call(this)) {
                    this.callback('redo',
                                 [this.point.geometry, this.getSketch(), true]);
                }
            }
