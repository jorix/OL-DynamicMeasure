...
OpenLayers.Control.DynamicMeasure = OpenLayers.Class(
    ...
    activate: function() {
        ...
        if (response) {
            ...
            this.layerHeading =
                            _create('labelHeading', this.layerLengthOptions);
    ...
    showLabelSegment: function(layer, index, fromIndex, points) {
        ...
        if (this.getBestLength(new OpenLayers.Geometry.LineString(points))[0] > 0)
        {
			var deltaX = to.x - from.x;
			var deltaY = to.y - from.y;
			var heading = Math.atan(deltaY/deltaX);
			var radToDeg = 180/Math.PI;
			
			if (deltaX >= 0 && deltaY >= 0) // + +
			{
				var bearing = (Math.PI/2 - heading)*radToDeg;
			}
			else if (deltaX >= 0 && deltaY < 0) // + -
			{
				var bearing = (Math.PI/2 + Math.abs(heading))*radToDeg;
			}
			else if (deltaX < 0 && deltaY < 0) // - -
			{
				var bearing = (3*Math.PI/2 - heading)*radToDeg;
			}
			else if (deltaX < 0 && deltaY >= 0) // - +
			{
				var bearing = (3*Math.PI/2 + Math.abs(heading))*radToDeg;
			}
